...
async function updateContentTypeConstraint() {
  ...
  try {
    ...
    if (dropError) {
      console.log('⚠️ Error eliminando restricción (puede que no exista):', dropError.message);
    } else {
    ...
    const constraintQuery = `
      ...
      CHECK (content_type IN (${allowedTypes.map(type => `'${type}'`).join(', ')}));
    `;
    ...
    if (testError) {
      if (testError.code === '23503') {
        console.log('✅ Restricción funciona correctamente (falla por FK como esperado)');
...
updateContentTypeConstraint().then((success) => {
  if (success) {
    ...
    console.log('💡 Ahora puedes usar los tipos: content, document, assignment, video, link, text');
  } else {
