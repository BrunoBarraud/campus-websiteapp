...
export const calendarService = {
  ...
  async getEvents(
    ...
    // Lógica de visibilidad basada en roles
    if (userRole === "student") {
      // Los estudiantes ven:
      ...
      // Los profesores ven:
      // 1. Eventos públicos y para profesores
      // 2. Sus propios eventos personales
      // 3. Eventos de materias que enseñan
      query = query.or(
        `visibility.eq.public,visibility.eq.teachers,and(created_by.eq.${userId},visibility.eq.private),subject_id.in.(select id from subjects where teacher_id = '${userId}')`
      );
    } else if (userRole === "admin") {
      // Los admins ven todos los eventos excepto los privados de otros usuarios
      query = query.or(
        `visibility.neq.private,and(created_by.eq.${userId},visibility.eq.private)`
      );
    }
