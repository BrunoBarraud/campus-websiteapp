...
export async function POST(
  ...
  try {
    ...
    const nextOrderIndex =
      maxOrder && maxOrder.length > 0 ? maxOrder[0].order_index + 1 : 1;
...
export async function DELETE(
  ...
  try {
    ...
    if (assignments && assignments.length > 0) {
      ...
      await supabaseAdmin.from("assignments").delete().eq("unit_id", unitId);
    }

    // Eliminar contenidos de la unidad
    await supabaseAdmin.from("subject_content").delete().eq("unit_id", unitId);
