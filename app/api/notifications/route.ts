...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: stats } = await supabaseAdmin
      .rpc('get_notification_stats', { target_user_id: currentUser.id });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validTypes = [
      'assignment_new', 'assignment_due_soon', 'assignment_graded',
      'assignment_comment', 'announcement', 'subject_new_content',
      'system', 'user_action'
    ];
