...
export async function GET(request: Request) {
  try {
    ...
    console.log('Student API: User accessing subjects:', currentUser.email, 'Year:', currentUser.year);
    ...
    console.log('Student API: Executing query for student_id:', currentUser.id, 'division:', currentUser.division);
    ...
    console.log('Student API: Raw enrollments data:', JSON.stringify(enrollments, null, 2));
    ...
    if (year) {
      subjects = subjects.filter(subject => subject && subject.year === parseInt(year));
      console.log('Student API: Subjects after year filter:', subjects.length, 'for year:', year);
    }
