...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: conversations, error } = await supabaseAdmin.rpc(
    ...
    if (type === "direct") {
      if (participants.length !== 1) {
        return NextResponse.json(
          {
            error:
              "Las conversaciones directas requieren exactamente un participante",
          },
          { status: 400 }
        );
      }
      ...
      if (currentUser.role === "student" && otherUser.role === "student") {
        return NextResponse.json(
          {
            error:
              "Los estudiantes no pueden iniciar conversaciones entre ellos",
          },
          { status: 403 }
        );
      }
      ...
      const { data: conversationId, error: createError } =
        await supabaseAdmin.rpc("create_direct_conversation", {
