...
export async function GET(request: NextRequest) {
  try {
    ...
    if (currentUser.role !== 'admin') {
      query = query.or(`target_audience.eq.${currentUser.role},target_audience.eq.all,target_audience.is.null`);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (target_roles && Array.isArray(target_roles)) {
      const invalidRoles = target_roles.filter(role => !validRoles.includes(role));
