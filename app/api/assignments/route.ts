...
export async function GET(request: NextRequest) {
  try {
    ...
    if (currentUser.role === "student") {
      ...
      query = query.eq("is_published", true).in("subject_id", subjectIds);
    ...
    if (currentUser.role === "student") {
      ...
      const assignmentsWithSubmissions = assignments.map((assignment) => ({
        ...assignment,
        submission:
          submissions?.find((s) => s.assignment_id === assignment.id) || null,
  ...
  try {
    ...
    if (is_published) {
      const { data: students } = await supabaseAdmin
        .from("enrollments")
        .select(
          "student_id, students:users!enrollments_student_id_fkey(id, name, email)"
        )
