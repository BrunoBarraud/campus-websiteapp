...
import { CalendarIcon, ClockIcon, UsersIcon, FileTextIcon, PlusIcon, EditIcon, TrashIcon } from "lucide-react";
...
export default function TeacherAssignmentsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(editingAssignment ? "Tarea actualizada exitosamente" : "Tarea creada exitosamente");
          ...
          {activeAssignments.length === 0 ? (
            ...
            <div className="grid gap-4">
              {activeAssignments.map((assignment) => (
                <Card key={assignment.id} className={`${isOverdue(assignment.due_date) ? 'border-red-200 bg-red-50' : ''}`}>
                  <CardHeader>
