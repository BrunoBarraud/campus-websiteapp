...
export default function SubjectHeroCard({
  ...
  const showNextDue = nextDueLabel !== undefined;
  ...
  return (
    <div className="mx-auto mb-6 max-w-4xl overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm sm:mb-8">
      ...
      {(showProgress || showNextDue) && (
        <div
          className={`grid gap-3 bg-white p-4 sm:p-5 ${
            showProgress && showNextDue
              ? "sm:grid-cols-[minmax(0,1.4fr)_minmax(280px,0.9fr)]"
              : "sm:grid-cols-1"
          }`}
        >
          {showProgress && (
