...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const Sidebar = ({ onNavigate }: { onNavigate?: () => void }) => (
    <div className="h-full bg-gradient-to-b from-white via-gray-50 to-gray-100 shadow-lg flex flex-col">
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-rose-950 to-yellow-500">
      ...
      <div className="flex flex-1">
        ...
        {/* Main content area */}
        <main className="flex-1 bg-gradient-to-br from-white via-gray-50 to-gray-100">
          {children}
        </main>
      </div>

      {/* Footer */}
      <footer className="bg-gradient-to-r from-rose-950 to-rose-800 text-amber-100 py-2 sm:py-3 text-center">
        <div className="px-4">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-t border-gray-200/50 z-50 shadow-lg pb-safe">
        <div className="grid grid-cols-5 py-1 sm:py-2">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
          ...
          <button
            onClick={handleLogout}
            className="flex flex-col items-center justify-center py-1 sm:py-2 px-1 text-xs transition-colors text-red-500 hover:text-red-700"
            title="Cerrar sesión"
          >
            <span className="text-sm sm:text-base md:text-lg mb-0.5 sm:mb-1">
              🚪
