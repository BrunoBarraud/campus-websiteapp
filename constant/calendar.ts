...
export const CALENDAR_CONFIG = {
  ...
  VISIBILITY_OPTIONS: {
    ADMIN: [
      { value: 'public' as EventVisibility, label: 'Público (Todos)', description: 'Visible para todos los usuarios' },
      { value: 'students' as EventVisibility, label: 'Estudiantes', description: 'Solo visible para estudiantes' },
      { value: 'teachers' as EventVisibility, label: 'Profesores', description: 'Solo visible para profesores' },
      { value: 'private' as EventVisibility, label: 'Privado', description: 'Solo visible para ti' }
    ],
    TEACHER: [
      { value: 'public' as EventVisibility, label: 'Público (Todos)', description: 'Visible para todos los usuarios' },
      { value: 'students' as EventVisibility, label: 'Para Estudiantes', description: 'Solo visible para estudiantes' },
      { value: 'private' as EventVisibility, label: 'Privado', description: 'Solo visible para ti' }
    ],
    STUDENT: [
      { value: 'private' as EventVisibility, label: 'Personal', description: 'Solo visible para ti' }
    ]
  },
...
export const CalendarUtils = {
  ...
  getEventTypesForRole: (role: string): EventType[] => {
    const roleKey = role.toUpperCase() as keyof typeof CALENDAR_CONFIG.EVENT_TYPES;
  ...
  getVisibilityOptionsForRole: (role: string) => {
    const roleKey = role.toUpperCase() as keyof typeof CALENDAR_CONFIG.VISIBILITY_OPTIONS;
  ...
  canCreateEvents: (role: string): boolean => {
    return CALENDAR_CONFIG.PERMISSIONS.CAN_CREATE_EVENTS[role as keyof typeof CALENDAR_CONFIG.PERMISSIONS.CAN_CREATE_EVENTS] || false;
  },

  // Verificar si un usuario puede editar sus propios eventos
  canEditOwnEvents: (role: string): boolean => {
    return CALENDAR_CONFIG.PERMISSIONS.CAN_EDIT_OWN_EVENTS[role as keyof typeof CALENDAR_CONFIG.PERMISSIONS.CAN_EDIT_OWN_EVENTS] || false;
  },

  // Verificar si un usuario puede editar todos los eventos
  canEditAllEvents: (role: string): boolean => {
    return CALENDAR_CONFIG.PERMISSIONS.CAN_EDIT_ALL_EVENTS[role as keyof typeof CALENDAR_CONFIG.PERMISSIONS.CAN_EDIT_ALL_EVENTS] || false;
  },
